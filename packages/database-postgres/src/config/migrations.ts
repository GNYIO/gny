...
import { fixLocalnetTransactionOrder } from './fixLocalnetTransactionOrder.js';
import { fixTestnetTransactionOrder } from './fixTestnetTransactionOrder.js';
import { fixMainnetTransactionOrder } from './fixMainnetTransactionOrder.js';
...
export class TransactionPlacement1740078090000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<any> {
    ...
    await queryRunner.query(`
      ALTER TABLE ONLY public.transaction ADD CONSTRAINT "transaction_height_placement_key" UNIQUE (height, placement);
    `);
  }
  async down(queryRunner: QueryRunner): Promise<any> {}
}

export class FixTransactionOrder1740614414000 implements MigrationInterface {
  async up(queryRunner: QueryRunner): Promise<any> {
    await queryRunner.query(`
      ALTER TABLE public.transaction DROP CONSTRAINT transaction_height_placement_key;
    `);

    // fix localnet
    await queryRunner.query(fixLocalnetTransactionOrder);
    // fix testnet
    await queryRunner.query(fixTestnetTransactionOrder);
    // mainnet
    await queryRunner.query(fixMainnetTransactionOrder);
