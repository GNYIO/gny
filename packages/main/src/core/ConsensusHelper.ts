...
export function doIncomingVotesFitInLine(
  oldState: IState,
  oldVotes: ManyVotes
) {
  const state = StateHelper.copyState(oldState);
  ...
  const pendingBlockId_is_pendingVotesid = pendingBlock.id === pendingVotes.id;
 ...
 * @param parentSpan
 */
export function addPendingVotes(
  oldState: IState,
  oldVotes: ManyVotes,
  parentSpan: ISpan
) {
...
export function hasPendingBlock(state: IState, timestamp: number) {
  const pendingBlock = state.pendingBlock;
  if (!pendingBlock) {
    return false;
  }
...
export function getPendingBlock(state: IState) {
  return state.pendingBlock;
}

export function clearState(old: IState) {
...
export function CollectingVotes(old: IState) {
  const state = StateHelper.copyState(old);
