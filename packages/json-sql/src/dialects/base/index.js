    ...
    return _(nameParts)
      .map(function(namePart) {
        if (
          ...
          namePart =
            self.config.identifierPrefix +
            namePart +
            self.config.identifierSuffix;
        }
...
Dialect.prototype.buildComparisonOperator = function(params) {
  ...
  var value = this.buildEndFetchingOperator({
    ...
    defaultFetchingOperator:
      operatorParams.defaultFetchingOperator || params.defaultFetchingOperator,
  });
...
Dialect.prototype.buildOperator = function(params) {
  ...
  if (contexts.logical && isContextValid(['null', 'logical'], context)) {
    ...
    contexts.fetching &&
    isContextValid(['logical', 'comparison'], context)
  ) {
    result = this.buildFetchingOperator(params);
  } else if (
    contexts.comparison &&
    isContextValid(['fetching', 'state'], context)
  ) {
...
Dialect.prototype.buildTemplate = function(type, params) {
  ...
  return template.pattern
    .replace(blockRegExp, function(fullMatch, block, space) {
