...
module.exports = function(dialect) {
  ...
  dialect.blocks.add('expression', function(params) {
    ...
    return expression.pattern
      .replace(/\{([a-z0-9]+)\}/gi, function(fullMatch, block) {
  ...
  dialect.blocks.add('join', function(params) {
    ...
    if (_.isArray(join)) {
      ...
      result = _(join)
        .map(function(joinItem, table) {
          if (
            !objectUtils.hasSome(joinItem, [
              'table',
              'query',
              'select',
              'expression',
  ...
  dialect.blocks.add('sort', function(params) {
    ...
    if (_.isArray(sort)) {
      ...
      result = _(sort)
        .map(function(direction, field) {
          return (
            dialect._wrapIdentifier(field) +
            ' ' +
