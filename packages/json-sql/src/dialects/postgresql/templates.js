...
module.exports = function(dialect) {
  ...
  dialect.templates.add('explain', {
    pattern:
      'explain {options} {analyze} {verbose} {query} {select} {expression}',
    validate: function(type, params) {
      ...
      if (!_.isUndefined(params.options)) {
        templateChecks.atLeastOneOfProps('explain:options', params.options, [
          'analyze',
          'verbose',
          'costs',
          'buffers',
  ...
  selectTemplate.pattern = selectTemplate.pattern.replace(
    '{distinct}',
    '{distinct} {distinctOn}'
  );
