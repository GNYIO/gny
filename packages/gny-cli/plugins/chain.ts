...
import * as valid_url from 'valid_url';

import * as accountHelper from '../helpers/account';
import * as dappHelper from '../helpers/dapp';
...
async function createChainMetaFile() {
  const answer = await prompt([
    ...
    },
    {
      ...
      name: 'delegates',
      message:
        "Enter public keys of chain delegates - hex array, use ',' for separator",
      validate: function(value) {
        const done = this.async();
        ...
        for (const i in publicKeys) {
          try {
            const b = new Buffer(publicKeys[i], 'hex');
            if (b.length != 32) {
...
async function depositChain() {
  ...
  const realAmount = parseFloat(
    (parseInt(result.amount) * 100000000).toFixed(0)
  );
  const body = {
...
async function withdrawalChain() {
  ...
  request(
    {
      url:
        'http://localhost:4096/api/chains/' + result.chain + '/api/withdrawal',
      method: 'post',
      json: true,
      body: body,
    },
...
async function createGenesisBlock() {
  ...
  const wantInbuiltAsset: any = await inquirer.prompt({
    type: 'confirm',
  ...
  const chainBlock = dappHelper.neww(account, null, assetInfo); // neww
...
export default function chain(program) {
  program
    .command('chain')
