...
const ZSchemaExpress = require("./utils/zscheme-express");
const Transaction = require("./base/transaction");
const Block = require("./base/block");
const Consensus = require("./base/consensus");
...
module.exports = function init(options, done) {
    ...
    async.auto({
        ...
        protobuf(cb) {
            ...
            protobuf.protobuf(protoFile, cb);
        },
        scheme(cb) {
        ...
        connect: ['config', 'genesisblock', 'logger', 'network', (cb, scope) => {
                ...
                scope.network.server.listen(scope.config.port, scope.config.address, (err) => {
                    ...
                    if (!err) {
                        if (scope.config.ssl.enabled) {
                            scope.network.sslserver.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, (sslError) => {
