...
priv.loop = (cb) => {
    ...
    return priv.getBlockSlotData(currentSlot, lastBlock.height + 1, (err, currentBlockData) => {
        ...
        return library.sequence.add(done => (() => __awaiter(this, void 0, void 0, function* () {
            try {
                if (slots.getSlotNumber(currentBlockData.time) === slots.getSlotNumber()
                    && modules.blocks.getLastBlock().timestamp < currentBlockData.time) {
                    yield modules.blocks.generateBlock(currentBlockData.keypair, currentBlockData.time);
                }
                done();
            }
            catch (e) {
...
shared.count = (req, cb) => (() => __awaiter(this, void 0, void 0, function* () {
    try {
...
shared.getVoters = (req, cb) => {
    ...
    library.scheme.validate(query, {
        ...
        return (() => __awaiter(this, void 0, void 0, function* () {
            try {
                const votes = yield app.sdb.findAll('Vote', { condition: { delegate: query.name } });
                if (!votes || !votes.length)
                    return cb(null, { accounts: [] });
                const addresses = votes.map(v => v.address);
                const accounts = yield app.sdb.findAll('Account', { condition: { address: { $in: addresses } } });
                const lastBlock = modules.blocks.getLastBlock();
                const totalSupply = priv.blockStatus.calcSupply(lastBlock.height);
