...
const priv = {
    ...
    initDHT: (p2pOptions) => __awaiter(this, void 0, void 0, function* () {
        p2pOptions = p2pOptions || {};
        let lastNodes = [];
        if (p2pOptions.persistentPeers) {
            const peerNodesDbPath = path.join(p2pOptions.peersDbDir, 'peers.db');
            try {
                lastNodes = yield promisify(priv.initNodesDb)(peerNodesDbPath);
                lastNodes = lastNodes || [];
        ...
        const bootstrapNodes = priv.getBootstrapNodes(p2pOptions.seedPeers, lastNodes, MAX_BOOTSTRAP_PEERS);
        const dht = new DHT({
