  ...
  it(
    'batch transactions',
      ...
      async function onComplete(event) {
        console.log(
          'Benchmark for batch transactions: ' + event.target.hz + ' ops/sec'
        );
      ...
      const suite = new Benchmark.Suite('BatchTransaction', {
      ...
      // add tests
      suite.add('BatchTransactions', {
        defer: true,
        fn: async function(deferred) {
          const trs = createTransactions(10);
          const data = {
            transactions: trs,
          };
          await axios.put(
